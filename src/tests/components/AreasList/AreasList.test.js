...
describe('AreasList', () => {
  ...
  describe('when there are no areas', () => {
    it("renders 'no areas' message", () => {
      store = mockStore(
        Store({ areasReducer: AreasReducerGenerator({ areas: [] }) }),
      );
      component = renderWithStore(store);
      expect(component.queryByAltText(i18n.t('no areas message'))).not.toBeInTheDocument();
    });
  });
